...
#url - scraped link
URL = f"https://shop.sprouts.com/api/v2/store_products?ads_enabled=true&ads_pagination_improvements=true&allow_autocorrect=true&limit={limit}&offset=0&search_provider=ic&search_term={items}&secondary_results=true&sort=rank&unified_search_shadow_test_enabled=false"
responces = requests.get(URL,headers = HEADERS)
data = responces.json()

print(data.keys())
print(data['items'][0]['name'])
print(data['items'][0]['base_price'])
