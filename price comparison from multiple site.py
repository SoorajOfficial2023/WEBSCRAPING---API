...
for admart_name in admart['name']:
    search_item = admart_name.lower()
    for competitor in competitior_list:
        URL = competitor.get('url')+search_item
        ...
        if responces.status_code == 200:
            data = responces.json()
            print(URL)
            print(data.get('items')[0].get('name'))
            print(data.get('items')[0].get('base_price'))
        else:
            print(f"error{responces.status_code}")
